// apps/setu_ngo/lib/features/dashboard/widgets/dashboard_widgets.dart
        ...
        children: [
          ...
          Row(children: [
            Icon(up ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded,
              size: 12, color: up ? kGreen : kRed),
            const SizedBox(width: 2),
            Text('${changePercent.abs().toStringAsFixed(0)}%',
              style: TextStyle(fontSize: 11, color: up ? kGreen : kRed, fontWeight: FontWeight.w600)),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(totalParticipants.toString(),
            style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: kTextDark)),
          const SizedBox(height: 2),
          Row(children: [
            Icon(changePercent >= 0 ? Icons.arrow_upward_rounded : Icons.arrow_downward_rounded,
              size: 13, color: kGreen),
            Text(' ${changePercent.abs().toStringAsFixed(0)}% from last month',
              style: const TextStyle(fontSize: 12, color: kGreen, fontWeight: FontWeight.w500)),
          ]),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: points.map((p) => Text(p.label,
              style: const TextStyle(fontSize: 10, color: kTextGrey))).toList(),
          ),
        ],
      ),
    );
...
class _LinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxY = points.map((p) => p.count).reduce(math.max).toDouble();
    final minY = points.map((p) => p.count).reduce(math.min).toDouble();
    final range = (maxY - minY) == 0 ? 1.0 : (maxY - minY);

    final dx = size.width / (points.length - 1);

    List<Offset> pts = List.generate(points.length, (i) => Offset(
      i * dx,
      size.height - ((points[i].count - minY) / range) * size.height,
    ));
    ...
    canvas.drawPath(fillPath, Paint()
      ..shader = LinearGradient(
        ...
        colors: [kPrimary.withOpacity(0.25), kPrimary.withOpacity(0.01)],
...
class DonutChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (categories.isEmpty) return const _EmptyChart(label: 'No category data yet');
...
// ── Heatmap ───────────────────────────────────────────────────────────────────
// Renders a proportional scatter plot of issue locations.
// Without a maps SDK, we draw a styled placeholder map with
// color-coded circles scaled by count and colored by severity.
...
class IssueHeatmap extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (points.isEmpty) {
      return Container(
        height: 200, decoration: _cardDec(),
        child: const Center(child: Text('No location data uploaded yet',
    ...
    return Container(
      ...
      child: Stack(children: [
        ...
        Positioned(
          ...
          child: Row(children: [
            const Text('Low Impact', style: TextStyle(fontSize: 10, color: kTextDark)),
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: const LinearGradient(
                    colors: [Color(0xFFFFEB3B), Color(0xFFFF5722), Color(0xFFB71C1C)]),
                ),
              ),
            ),
            const Text('High Impact', style: TextStyle(fontSize: 10, color: kTextDark)),
          ]),
        ),
      ]),
    );
...
class _HeatmapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Normalize lat/lng to canvas coords
    ...
    final latRange = (maxLat - minLat).abs() == 0 ? 1.0 : (maxLat - minLat);
    final lngRange = (maxLng - minLng).abs() == 0 ? 1.0 : (maxLng - minLng);

    final maxCount = points.map((p) => p.count).reduce(math.max);

    for (final point in points) {
      final x = ((point.longitude - minLng) / lngRange) * (size.width  * 0.8) + size.width  * 0.1;
      final y = ((maxLat - point.latitude)  / latRange) * (size.height * 0.7) + size.height * 0.1;

      final radius = 12.0 + (point.count / maxCount) * 28;
      final color  = _severityColor(point.severity);

      // Glow
      canvas.drawCircle(Offset(x, y), radius * 1.8,
  ...
  Color _severityColor(Severity s) => switch (s) {
    Severity.low    => const Color(0xFFFFEB3B),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 2),
        Row(children: [
          const Icon(Icons.location_on_outlined, size: 13, color: kTextGrey),
          const SizedBox(width: 2),
        ...
        const SizedBox(height: 4),
        Text('${(campaign.goalPercent * 100).toStringAsFixed(0)}% Goal Achieved',
...
class CampaignTile extends StatelessWidget {
        ...
        children: [
          ...
          Container(
            ...
            child: campaign.imageUrl != null
              ...
              : Icon(Icons.campaign_rounded, color: kPrimary, size: 28),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  Container(
                    ...
                    child: const Text('Upcoming',
                      style: TextStyle(fontSize: 10, color: kGreen, fontWeight: FontWeight.w600)),
                  ),
                ]),
                const SizedBox(height: 4),
                Row(children: [
                ...
                const SizedBox(height: 2),
                Row(children: [
                  const Icon(Icons.calendar_today_outlined, size: 12, color: kTextGrey),
                  const SizedBox(width: 2),
                  Text(dateStr, style: const TextStyle(fontSize: 11, color: kTextGrey)),
                ]),
                const SizedBox(height: 4),
                Text('${campaign.volunteerCount}/${campaign.volunteerGoal} Volunteers',
  ...
  String _time(DateTime d) => '${d.hour.toString().padLeft(2,'0')}:${d.minute.toString().padLeft(2,'0')} AM';
}
...
class _PillTag extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 4),
      const Icon(Icons.keyboard_arrow_down_rounded, size: 14, color: kTextGrey),
    ]),
  );
}
...
class _EmptyChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(
    height: 180, decoration: _cardDec(),
    child: Center(child: Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        const Icon(Icons.bar_chart_rounded, color: kTextGrey, size: 36),
        const SizedBox(height: 8),
